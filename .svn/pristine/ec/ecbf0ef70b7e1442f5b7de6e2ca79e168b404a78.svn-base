...
public class AFN19ResultDataBackLisener implements AFN19DataBackLisener {
    ...
    @Autowired
    ChargingDevlogService chargingDevlogService;
    @Autowired
    ...
    private void sendAppUser(AFN19Object afn19Object) {
        ...
        try{
            ...
            if(ChargeConstant.OnOffType.OFF.getType().equalsIgnoreCase(powerState) && status == 1){
                ...
                // 中间件来算
//                appChargerService.stopCharge(chargingGuid);

                // 断电发送短信
                try {
                    //根据chagingGuid查询日志警告记录
                    ChargingDevlog chargingDevlog = chargingDevlogService.queryRecentAlarm(chargingGuid);
                    if(chargingDevlog!=null){
                        // 异常断电发送告警短信
//                    logger.info("开始向用户发送告警信息");
                        appChargerService.sendAlarmMessage(chargingDevlog,chargingDevice.getInstallAddr(),afn19Object.getTime());
                    }else{
                        // 正常断电发送告警短信
//                    logger.info("开始向用户发送断电信息");
                        appChargerService.sendStopMessage(chargingGuid,chargingDevice.getInstallAddr(),afn19Object.getTime());
                    }
                } catch (Exception e) {
                    logger.error("断电发送短信异常",e);
                }
            } else if(ChargeConstant.OnOffType.ON.getType().equalsIgnoreCase(powerState) && status==1){
                //费用扣减操作
